...
const useHabitacion = (initialSelection) => {
  const [habitacionSeleccionada, setHabitacionSeleccionada] = useState(initialSelection || null);

  const seleccionarHabitacion = (id) => setHabitacionSeleccionada(id);
  const limpiarSeleccion = () => setHabitacionSeleccionada(null);

  return { habitacionSeleccionada, seleccionarHabitacion, limpiarSeleccion };
};
...
const HabitacionCard = ({ habitacion, isSelected, onSelect }) => (
  <div
    onClick={() => onSelect(habitacion.id)}
  ...
  >
    <div className="relative h-32 overflow-hidden">
      <img src={habitacion.imagen} alt={habitacion.nombre} className="w-full h-full object-cover" loading="lazy" />
    ...
    <div className="p-2">
      <div className="flex justify-between items-start">
        ...
        <span className="text-[#7CB936] font-medium text-xs whitespace-nowrap ml-1">{habitacion.precio}</span>
      </div>

      <div className="flex items-center mt-1 text-xs text-[#364153]">
        {habitacion.internet && <WiFiIcon />}
        <span className="ml-1">{habitacion.rating}</span>
      </div>

      <p className="text-xs text-[#6A7282] mt-1 line-clamp-2">{habitacion.descripcion}</p>

      <div className="grid grid-cols-2 gap-1 mt-2 text-xs text-[#6A7282]">
        <div className="flex items-center"><SizeIcon /> {habitacion.tamaño}</div>
        <div className="flex items-center"><CapacityIcon /> {habitacion.capacidad}</div>
      </div>

      <div className="mt-2">
        <span className={`text-xs ${habitacion.disponibles > 0 ? 'text-[#1C3458]' : 'text-red-600'}`}>
          {habitacion.disponibles > 0 ? `${habitacion.disponibles} disponibles` : 'Agotado'}
        </span>
      </div>
    </div>
  </div>
...
export default function StepHabitaciones({ datos, onSiguiente, onVolver, onClose }) {
  const { habitacionSeleccionada, seleccionarHabitacion } = useHabitacion(datos.habitacion);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-2">
            {habitaciones.map((habitacion) => (
              <HabitacionCard
                ...
                onSelect={seleccionarHabitacion}
              />
            ))}
          </div>
        </div>
